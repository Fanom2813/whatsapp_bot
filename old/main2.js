...
import { getActiveConversationsCount, getConversationStatus, clearUserConversation, getUserMessages, setUserMessages } from '../rag-system.js';
import { functionDefinitions, executeFunctionCall } from './function-tools.js';
import SimpleRAG from '../simple_rag.js';
...
// Initialize SimpleRAG instance at startup
const simpleRAG = new SimpleRAG();
...
// Function to answer questions using SimpleRAG
async function answerWithSimpleRAG(message, userId) {
    try {
        // Get conversation messages for context
        let messages = getUserMessages(userId) || [];

        // Get relevant context from SimpleRAG
        const context = await simpleRAG.generateContext(message, 3);
...
The customer's phone number is: ${userId}`
            });
        }
        ...
        console.log(`💾 Updated RAG conversation for user ${userId} (${messages.length} messages)`);
        return assistantResponse;

    } catch (error) {
        console.error('Error in SimpleRAG response:', error);
...
async function generateResponse(message, customerPhone) {
    try {
        ...
        if (requiresFunctionCall(message)) {
            ...
            // Use SimpleRAG for general knowledge queries with user ID for conversation continuity
            return await answerWithSimpleRAG(message, userId);
        }
    } catch (error) {
    ...
    try {
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: `You are an AI assistant for Babu Motors Uganda. Here's relevant context: ${context}`
                },
                {
                    role: "user",
                    content: message
...
client.on('ready', () => {
    ...
    console.log('📚 SimpleRAG system loaded with:', simpleRAG.getStats());
});
